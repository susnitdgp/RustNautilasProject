...
An experimental hybrid Chandelier fallback is implemented but intentionally disabled in production. When enabled, WaveTrend retains first ownership of strong-momentum trades: once the existing WT arm is reached, Chandelier is suppressed for the remainder of that position and the normal WT pullback/slope exit remains authoritative. If WT has not armed, Chandelier can become active only after the trade has first achieved the configured favorable excursion (current candidate 1.5 ATR). It then trails the trade-specific post-entry peak/trough by the configured ATR multiple (current candidate 3.0 ATR), with a breakeven floor based on Nautilus `avg_px_open`. The actor refreshes that authoritative entry price from the live cached position so reconnect/rebuild state does not depend on an approximate LTP entry.

With the hybrid enabled, event priority is WaveTrend exit, Chandelier fallback exit, FAST reversal, pre-close reversal, then the confirmed Trend Ribbon transition. Both WT and Chandelier exits target FLAT only and block same-trend confirmed-close synchronization until the confirmed direction changes. Historical candle tests use the same ownership and trailing rules on completed five-minute bars, so they are only a proxy for realtime intrabar stop timing.

Only one strategy event is admitted per candle. Reversal targets use the existing Nautilus/Kite reducing-then-entering order path. The causal trigger is retained across both legs of a split reversal. FAST/pre-close reversals are reconciled back to the persistent confirmed direction on a later confirmed bar if the intrabar reversal does not survive confirmation.
