...
## September 16: manual-run validation and optional Slack alerts
User selected manual terminal launch; do not install an Ubuntu service. See doc/UnattendedValidation.md for fault-test evidence and remaining unattended-production limits. Rust Slack notifications are optional and disabled by default; see doc/SlackAlerts.md. No Slack request or real order was sent. The supplied webhook was not saved; provision a rotated secret separately. Release verification now preserves a unique evidence directory on every rerun.

User decision: independent hard-crash monitoring is deferred; do not implement it in this checkpoint. Existing crash/restart regression tests remain included.
