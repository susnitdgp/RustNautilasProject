...
attempt=candidate/("verification_attempt_"+str(uuid.uuid4()))
attempt.mkdir()
...
(attempt/"preflight.stdout.log").write_text(check.stdout)
(attempt/"preflight.stderr.log").write_text(check.stderr)
...
report=attempt/"replay_2026-09-15"
...
(attempt/"verification.stdout.log").write_text(run.stdout)
(attempt/"verification.stderr.log").write_text(run.stderr)
...
manifest["verification"]={"evidence_directory":str(attempt),"preflight_blocks_activation":True,"release_replay_matches_reference_exactly":True,"date":"2026-09-15","trades":6,"fills":12,"gross_pnl_inr":21800,"open_contracts":0,"stop_orders":0}
