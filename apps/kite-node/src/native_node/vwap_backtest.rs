...
pub fn run(date: &str, path: &str, folder: &str) -> Result<()> {
    ...
    if let Err(error) = &outcome {
        report::json(
            folder,
            "failure.json",
            &serde_json::json!({"status":"failed","error":error.to_string(),"live_orders_enabled":false}),
        )?;
    }
    outcome.map(|value| println!("{value}"))
}
pub fn execute(
    date: NaiveDate,
    data: Input,
...
mod tests {
    use super::*;
    #[test]
    fn native_vwap_gap_stop_uses_open() {
        if std::env::var_os("KITE_VWAP_GAP_CHILD").is_none() {
            let out = std::process::Command::new(std::env::current_exe().unwrap())
                .args([
                    "--exact",
                    "native_node::vwap_backtest::tests::native_vwap_gap_stop_uses_open",
                    "--nocapture",
                ])
                .env("KITE_VWAP_GAP_CHILD", "1")
                .output()
                .unwrap();
            assert!(
                out.status.success(),
        ...
        let mut policy = super::super::vwap_signal::Policy::new();
        let mut setup = None;
    ...
    #[test]
    fn native_vwap_stops_and_flatness() {
        if std::env::var_os("KITE_VWAP_TEST_CHILD").is_none() {
            let out = std::process::Command::new(std::env::current_exe().unwrap())
                .args([
                    "--exact",
                    "native_node::vwap_backtest::tests::native_vwap_stops_and_flatness",
                    "--nocapture",
                ])
                .env("KITE_VWAP_TEST_CHILD", "1")
                .output()
                .unwrap();
            assert!(
                out.status.success(),
