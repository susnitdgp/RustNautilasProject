//! Same-source five versus ten-minute confirmed Supertrend comparison without stops.
...
pub fn run(start: &str, end: &str, path: &str) -> Result<()> {
    ...
    let folder = report::directory(
        &format!("supertrend_interval_comparison_{start}_to_{end}"),
        &UUID4::new().to_string(),
    )?;
...
fn execute(
    ...
    let data = supertrend_input::load(end, Some(path))?;
        ...
        "Supply the original five-minute input"
    );
    let aggregated = super::ten_minute::aggregate(&data)?;
    std::fs::copy(path, folder.join("historical_input.json"))?;
    report::json(folder, "ten_minute_input.json", &aggregated)?;
    let mut rows = Vec::new();
    for data in [&data, &aggregated] {
        let variant = data.interval.as_str();
        ...
        for date in days {
            ...
            vwap_input::validate_interval(&session, *date)?;
            report::json(&dir, &format!("{date}_input.json"), &session)?;
        }
        ...
        for date in days {
            ...
            let output = std::process::Command::new(std::env::current_exe()?)
                ...
                .arg(date.to_string())
                .arg(dir.join(format!("{date}_input.json")))
                .arg(&child)
                .arg("confirmed")
                .output()?;
            let logs = format!(
        ...
        let row = json!({"variant":variant,"interval":variant,"statistics":stats,"sessions":daily});
    ...
    let output = json!({"status":"completed","start_date_ist":start.to_string(),"end_date_ist":end.to_string(),"sessions":days.len(),
        "instrument":data.instrument_id,"intervals":["5minute","10minute"],"contracts":1,"contract_multiplier":100,
    ...
    let mut md = format!(
        "# Confirmed Supertrend: five versus ten-minute candles\n\n{start} through {end}; {} sessions; 5-minute versus 10-minute September CRUDEOIL; one lot.\n\n| Variant | Trades | Wins | Gross INR | Closed-trade drawdown INR |\n|---|---:|---:|---:|---:|\n",
        days.len()
    );
    for r in &rows {
        let s = &r["statistics"];
