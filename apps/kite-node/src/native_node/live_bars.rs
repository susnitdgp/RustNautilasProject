...
pub fn completed_for(candles: Vec<Candle>, now: u64, interval: Interval) -> Result<Vec<Candle>> {
    ...
    let mut out = Vec::new();
    for c in candles {
        if close_for(&c, interval)? <= now.saturating_sub(super::bar_timing::COMPLETION_GRACE_NS) {
            out.push(c);
        }
    }
...
mod tests {
    ...
    fn partial_bars_duplicates_gaps_and_revisions_are_handled() {
        let all = super::super::test_support::two_day_candles();
...
fn warmup_requires_complete_sessions_and_latest_closed_candle() {
    ...
    let (start, _) = super::test_support::bounds(date);
    let now = start + 3 * STEP + 2_000_000_000;
    let (previous, end) = super::test_support::bounds(date.pred_opt().unwrap());
