//! Requested trading sessions with isolated native engines and a combined report.
...
pub fn run(end: &str, path: Option<&str>) -> Result<()> {
    ...
    let result = execute(end, path, &days, &folder, Variant::Baseline);
...
pub fn execute(
    ...
    md.push_str(&format!(
        "\nEntry variant: {}. See VwapFilterComparison.md for exact variant rules.\n",
        variant.name()
    ));
