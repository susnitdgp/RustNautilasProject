...
use super::{supertrend_actor::State, supertrend_stop_policy::Policy, vwap_signal::stop_price};
...
impl StopStrategy {
    pub fn new(
        ...
        Self {
            core: StrategyCore::new(StrategyConfig {
                strategy_id: Some("SUPERTREND-STOP-001".into()),
                log_events: false,
                log_commands: false,
                ..Default::default()
            }),
            policy: Policy::new(blocked, start),
            bar_type,
            start,
            end,
            state,
    ...
    fn on_fill(&mut self, event: &OrderFilled) -> Result<()> {
        ...
        if reason == "stop_loss" {
            self.policy.stopped();
            self.state.borrow_mut().blocked_direction = self.policy.blocked_direction;
            self.queued = None;
        }
...
impl DataActor for StopStrategy {
...
nautilus_strategy!(StopStrategy, {
